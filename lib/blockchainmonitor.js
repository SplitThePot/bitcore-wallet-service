...
BlockchainMonitor.prototype._handleIncomingPayments = function(coin, network, data) {
  ...
  var localBuffer = this.buffer.splice(0, 100);

  this.storage.fetchAddressesByInfo(localBuffer, function(err, dbAddresses) {
    handleAddreses(err, dbAddresses, localBuffer);
  });

  function handleAddreses(err, dbAddresses, buffer) {
    if (err) {
      ...
      if (err.message.indexOf('Request rate is large') >= 0) {
        ...
        setTimeout(function() {
          self.storage.fetchAddressesByInfo(buffer, function(err, dbAddresses) {
            handleAddreses(err, dbAddresses, buffer);
          });
        }, 100);
      }
      return;
    ...
    var walletIds = _.uniq(_.map(dbAddresses, 'walletId'));
    ...
    function runAsync(dbAddresses, buffer) {
      async.each(
        ...
        function(dbAddr, next) {
          if (!dbAddr || dbAddr.isChange) return next();
          ...
          _.forEach(allTxForAddress, function(txForAddress) {
            var alreadyNotified = _.any(walletNotifications, function(n) {
              return n.type == 'NewIncomingTx' && n.data && n.data.txid == txForAddress.txid;
            });
            if (alreadyNotified) {
