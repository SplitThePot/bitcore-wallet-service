...
BlockchainMonitor.prototype._handleThirdPartyBroadcasts = function(data, processIt) {
  ...
  self.storage.fetchTxByHash(data.txid, function(err, txp) {
    if (err) {
      log.error(`Could not fetch tx from the db, transactionId:${data.txid}, error:${err}`);
      return;
    }
...
BlockchainMonitor.prototype._handleIncomingPayments = function(coin, network, data) {
  ...
  log.info(`Handle incoming payments, count:${outs.length}`);
  ...
  async.each(
    ...
    function(out, next) {
      ...
      self.storage.fetchAddressByCoin(coin, out.address, function(err, address) {
        if (err) {
          log.error(`Could not fetch addresses from the db, coin:${coin}, address:${out.address}, error: ${JSON.stringify(err)}`);
          return next(err);
        }
